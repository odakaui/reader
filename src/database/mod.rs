...
* `index` - The index of the word in the current line that is visible in the reader
* `line` - The current line that is visible in the reader
* `order` - An integer representing the order of the states
...
* `known` - The total number of times a token was marked as known by the user during a given read through
