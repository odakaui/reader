...
use crate::{ApplicationState, StatisticsState, TokenInfo, Status};
use druid::widget::{Container, Either, Flex, Label, List, Scroll};
use druid::{Env, FontDescriptor, FontFamily, Insets, Widget, WidgetExt, LensExt};
...
pub fn build_data_view() -> impl Widget<ApplicationState> {
    ...
    let header_label = Label::new(|data: &StatisticsState, _: &Env| {
        format!("Statistics for {}", data.name)
    })
    .with_font(primary_font)
    .lens(ApplicationState::statistics_state);

    let start_label = Label::new(|data: &StatisticsState, _: &Env| {
        format!("Started {}", data.start_date)
    })
...
fn format_info(info: &TokenInfo) -> String {
    format!("{}, {} (marked as known {}% of the time)", info.total_unknown(), info.lemma(), info.percent_known())
}
